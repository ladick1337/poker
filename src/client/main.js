...
function renderOpponent(name, data) {
  ...
  if (data.buyIns > 0) {
    if (data.text == 'Fold') {
      console.log('data.pmatches :'+data.pmatches);
      console.log('data.pwins :'+data.pwins);
      return (
...
$('#gameDiv .row #opponentCards').on('mouseover', '.card-content span.card-title',function() {
  $('.card-content .player-stats.active').removeClass('active');

  $(this).parent().find('.player-stats').addClass('active');
  // console.log("this :" + $(this));
  // console.log("hover");
});

$('#opponentCards').on('mouseout', '.card-content', function(){
  $('.card-content .player-stats.active').removeClass('active');
})
